...
(use-package
  pyenv-mode
  :init (let ((pyenv-path (expand-file-name "~/.pyenv/bin")))
...
(use-package
  python-pytest
  :init (setq pytest-cmd-flags "--pdbcls=IPython.terminal.debugger:Pdb")
  :custom (python-pytest-confirm t)
...
(use-package
  ...
  (setq pyvenv-post-activate-hooks (list (lambda ()
                                           (setq python-shell-interpreter (concat pyvenv-virtual-env
                                                                                  "bin/python")))))
  (setq pyvenv-post-deactivate-hooks (list (lambda ()
...
(defun db/add-lsp-workspaces ()
  (let* ((project-path (projectile-project-root))
         ...
         (workspace-folders-to-add `(,project-path ,db/venv-path ,db/actual-venv-path
                                                   "/home/dbohomiakov/.pyenv/")))
    (progn (print db/pname)
           (setq lsp-pyright-venv-directory db/pname)
           (mapcar 'lsp-workspace-folders-remove workspace-folders-to-remove)
           (mapcar 'lsp-workspace-folders-add workspace-folders-to-add))))

(defun db/run-lsp ()
  (interactive)
  (let ((lsp-activated? (lsp-session-folders (lsp-session))))
    (progn (when lsp-activated? (lsp-workspace-shutdown))
           (db/add-lsp-workspaces))))

(defun db/enable-lsp ()
  (when db/lsp-enable? (db/run-lsp)))

(defun db/enable-python-venv ()
  (let ((pname db/pname))
    (progn (pyvenv-workon pname)
           (unless (file-directory-p pyvenv-virtual-env)
             (progn (pyvenv-deactivate)
                    (call-interactively #'pyvenv-workon))))))

(defun db/configure-python-project ()
  (progn (db/enable-python-venv)
         (db/enable-lsp)))

(defun db/do-nothing ()
  nil)

(setq db/configure-fn-by-lang '(("python" . db/configure-python-project)
                                (nil . db/do-nothing)))
...
(defun db/py-isort-before-save ()
  (when db/auto-isort (py-isort-before-save)))

(defun db/pyimport-remove-unused ()
  (when db/remove-unused-imports (pyimport-remove-unused)))
...
(fset 'ipdb (kmacro-lambda-form [?i ?m ?p ?o ?r ?t ?  ?i ?p ?d ?b ?\; ?i ?p ?d ?b ?. ?s ?e ?t ?_ ?t
                                    ?r ?a ?c ?e ?\( ?c ?o ?n ?t ?e ?x ?t ?= ?5 ?0 escape] 0 "%d"))
